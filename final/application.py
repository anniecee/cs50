...
@app.route("/")
def index():
    """Show lists of current currency"""
...
def convert():
    """Convert currency"""
    # User reached route via POST (as by submitting currencies and amount)
    if request.method == "POST":

        # Ensure currencies were entered (value in form of Country Id)
        if not request.form.get("input_cur") or not request.form.get("output_cur"):
            return apology("must provide types of currency", 411)
        
        # Ensure the amount to be converted was entered
        elif not request.form.get("amount")
            return apology("must provide amount", 411)

        # Ensure entered amount was valid
        elif int(request.form.get("amount")) < 0:
            return apology("invalid amount", 411)

        # If input are valid,
        else:
            # Save input information 
            input_cur_id = lookup(request.form.get("input_cur"))['id']
            input_cur_symbol = lookup(request.form.get("input_cur"))['symbol']
            
            output_cur_id = lookup(request.form.get("output_cur"))['id']
            output_cur_symbol = lookup(request.form.get("output_cur"))['symbol']
            
            amount = int(request.form.get("amount"))

            # Get exchange rate:
            ex_rate = get_rate(input_cur_id, output_cur_id)
            
            # Calculate converted amount in output currency:
            conv_amount = amount*ex_rate
            
            #Format amount for outputs:
            amount_display = format_cur(amount, input_cur_id, input_cur_symbol)
            conv_amount_display = format_cur(conv_amount, input_cur_id, input_cur_symbol)

            # Flash message
            flash("Converted!")

            # Render template
            return render_template("converted.html", amount = amount_display, conv_amount = conv_amount_display)
    ...
    # User reached route via GET (as by clicking a link or via redirect)
        ...
        for i in country_list:
            ...
            cur_display.append(cur_list)
            
            
        return render_template("convert.html", cur_list = cur_list)
